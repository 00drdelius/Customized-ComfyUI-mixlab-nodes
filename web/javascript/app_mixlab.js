...
/**
 * 
 * @param jsonData await app.graphToPrompt
 * @param inputIds input ids selected in Mixlab AppInfo
 * @param outputIds output ids selected in Mixlab AppInfo
 * @returns  will be loaded into app in mixlab workflow json 
 */
    ...
    if (data.hasOwnProperty(id)) {
      ...
      if (inputIds.includes(id)) {
        ...
        if (node.type === "KSampler"){
          const arr=['combo','number']
          options = node.widgets.filter(w => arr.includes(w.type))
        }
        if (node.type === "PhotoMakerLoader"){
          options = node.widgets
        }
        if (node.type === "PhotoMakerEncodePlus"){
          options = null
        }
        // **update**
